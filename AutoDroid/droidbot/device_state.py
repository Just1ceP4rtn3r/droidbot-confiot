...
class DeviceState(object):
    ...
    def _save_important_view_ids(self):
        ...
        important_view_id_path = (
            self.device.output_dir + "/states_view_ids/" + self.state_str + ".txt"
        )
    ...
    def _merge_text(self, view_text, content_description):
        ...
        if content_description:
            ...
            content_description = (
                f"{content_description[:20]}..."
                if len(content_description) > 20
                else content_description
            )
    ...
    def get_described_actions_bk(self, prefix=""):
        ...
        for view_id in enabled_view_ids:
            ...
            if (
                not content_description and not view_text and not scrollable
            ):  # actionable?
                continue
            ...
            if editable:
                ...
                view_desc = (
                    btn_frame.replace("@", str(len(view_descs)))
                    .replace("#", view_text)
        ...
        for view_id in range(1, len(self.views)):
            if self.__safe_dict_get(self.views[view_id], "clickable", default=False):
                ...
                for successor in successors:
                    if successor != view_id and self.__safe_dict_get(
                        self.views[successor], "clickable", False
                    ):
            ...
            if self.__safe_dict_get(self.views[view_id], "checkable", default=False):
                ...
                for successor in successors:
                    if successor != view_id and self.__safe_dict_get(
                        self.views[successor], "checkable", False
                    ):
    ...
    def _merge_textv2(
        self, children_ids, remove_time_and_ip=False, important_view_ids=[]
    ):
        texts, content_descriptions = [], []
        for childid in children_ids:

            if not self.__safe_dict_get(
                self.views[childid], "visible"
            ) or self.__safe_dict_get(self.views[childid], "resource_id") in [
                "android:id/navigationBarBackground",
                "android:id/statusBarBackground",
            ]:
    ...
    def _has_clickable_children(self, id):
        ...
        for child_view_id in children:
            clickable = self.__safe_dict_get(
                self.views[child_view_id], "clickable", default=False
            )
            checkable = self.__safe_dict_get(
        ...
        for view_id in enabled_view_ids:
            ...
            if editable:
                ...
                if merge_buttons:
                    ...
                    view_text, content_description, important_view_ids = (
                        self._merge_textv2(
