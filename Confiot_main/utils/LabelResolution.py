...
class Vector:
    ...
    def get_magnitude(self):
        if (self.start.x == 0 and self.start.y == 0 and self.end.x == 0 and self.end.y == 0):
            return -1
...
def calc_collision_vector_w(r1: Rectangle, r2: Rectangle, is_above: bool) -> List[Vector]:
    v1 = Vector(r1.get_top_left() if is_above else r1.get_bottom_left(),
                r2.get_bottom_left() if is_above else r2.get_top_left(), 90.0 if is_above else 270.0)
    v2 = Vector(r1.get_top_right() if is_above else r1.get_bottom_right(),
                r2.get_bottom_right() if is_above else r2.get_top_right(), 90.0 if is_above else 270.0)
...
def is_entirely_above_or_below(r1: Rectangle, r2: Rectangle) -> bool:
    return (r1.left >= r2.left - DISTANCE_THRESHOLD and
            r1.right <= r2.right + DISTANCE_THRESHOLD) or (r2.left >= r1.left - DISTANCE_THRESHOLD and
                                                           r2.right <= r1.right + DISTANCE_THRESHOLD)
...
def get_common_coordinates_w(r1: Rectangle, r2: Rectangle, is_above: bool) -> Tuple[Coordinate, Coordinate]:
    ...
    while i < r1.right:
        if r2.left <= i <= r2.right:
            break
        i += 1
    return (Coordinate(i, r1.top if is_above else r1.bottom), Coordinate(i, r2.bottom if is_above else r2.top))


def calc_collision_vector_h(r1: Rectangle, r2: Rectangle, is_left: bool) -> List[Vector]:
    v1 = Vector(r1.get_top_left() if is_left else r1.get_top_right(),
                r2.get_top_right() if is_left else r2.get_top_left(), 180.0 if is_left else 0.0)
    v2 = Vector(r1.get_bottom_left() if is_left else r1.get_bottom_right(),
                r2.get_bottom_right() if is_left else r2.get_bottom_left(), 180.0 if is_left else 0.0)
    ...
    if is_entirely_left_or_right(r1, r2):
        ...
        v4 = Vector(Coordinate(-1, 0), Coordinate(10000, 0), 0)
        if USE_THIRD_VECTOR:
...
def is_entirely_left_or_right(r1: Rectangle, r2: Rectangle) -> bool:
    return (r1.top >= r2.top - DISTANCE_THRESHOLD and
            r1.bottom <= r2.bottom + DISTANCE_THRESHOLD) or (r2.top >= r1.top - DISTANCE_THRESHOLD and
                                                             r2.bottom <= r1.bottom + DISTANCE_THRESHOLD)
...
def get_common_coordinates_h(r1: Rectangle, r2: Rectangle, is_left: bool) -> Tuple[Coordinate, Coordinate]:
    ...
    while i < r1.bottom:
        if r2.top <= i <= r2.bottom:
            break
        i += 1
    return (Coordinate(r1.left if is_left else r1.right, i), Coordinate(r2.right if is_left else r2.left, i))
